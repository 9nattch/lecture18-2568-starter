...
router.get("/",authenticateToken,checkRoleAdmin, (req: CustomRequest, res: Response) => {
   try {
//     //get Authrization headers
//     const authHeader = req.headers["authorization"]
//     console.log(authHeader)
//     //if authHeader is not found or wrong formet
...
//             }
            // console.log(payload);
            //exxtract "user" payload from CustomRequest
        //     const payload = req.user;
        //     const user = users.find(
        //     (u:User) => u.username === (payload as UserPayload).username
        // );

        // //check if user exist and has role ADMIN
...
router.post("/login", (req: Request, res: Response) => {
    try{//1. get username and password from body
